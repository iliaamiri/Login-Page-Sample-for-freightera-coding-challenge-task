...
class user extends model
{
    ...
    {
        $table = array('witcher_users', 'witcher_witcher_user_permissions');
    ...
    {
        $table = ['witcher_users', 'witcher_user_permissions'];
    ...
    {
        $sql = parent::$db->mdb_query("SELECT * FROM witcher_users $statements", 1);
        return $sql;
    }
    ...
    public function user_exist($user_name)
    {
        ...
        if ($preg_user) {
            $sql = parent::$db->mdb_query("SELECT * FROM witcher_users WHERE Username = '$user_name'", 1);
            if ($sql->rowCount() > 0) {
                return true;
            }
        }
    ...
    {
        $sql = parent::$db->mdb_select('witcher_users', '*', "WHERE user_second_id = '" . $user_second_id . "'");
    ...
    public function email_exist($email)
    {
        ...
        if ($preg_user) {
            $sql = parent::$db->mdb_query("SELECT * FROM witcher_users WHERE Email = '$email'", 1);
            if ($sql->rowCount() > 0) {
                return true;
            }
        }
    ...
    public function AddUser($data, $guest = false)
    {
        ...
        try {
            ...
            parent::$db->mdb_query("INSERT INTO witcher_users (Username,Email,Password,user_second_id) VALUE ('$data[Username]','$data[Email]','$data[Password]','$user_second_id')", 1);
    ...
    {
        $sql = parent::$db->mdb_query("SELECT * FROM witcher_users", 1);
        return $sql->rowCount();
    }

    public function CountUsersBy($column, $value)
    {
        $sql = parent::$db->mdb_query("SELECT * FROM witcher_users WHERE $column = '$value'", 1);
        return $sql->rowCount();
    }
    ...
    public function UpdateUserTbl($password, $image, $Email, $newEmail = "", $newFull = "", $newusername = "")
    {
        ...
        parent::$db->mdb_query("UPDATE witcher_users SET Password = '$password', Profile_Image = '$image' $custom $custom2 $custom3 WHERE Email = '$Email'", 1);
    }

    public function UpdateUserTblCustom($statements)
    {
        parent::$db->mdb_query("UPDATE witcher_users SET $statements", 1);
        return true;
    }

    public function UpdateColumn($column, $value, $statement)
    {
        parent::$db->mdb_query("UPDATE witcher_users SET $column = '$value' $statement", 1);
        return true;
    }
    ...
    {
        $result = [];
        $sql_users = parent::$db->mdb_query("SELECT * FROM witcher_users", 1);
        foreach ($browsers_list as $browser) {
            $sql = parent::$db->mdb_query("SELECT * FROM witcher_users WHERE Last_Browser = '$browser'", 1);
